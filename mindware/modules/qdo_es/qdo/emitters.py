...
class DiscreteWeightSpaceEmitter(EmitterBase):
    ...
    def tell(self, solution, objective, measures, metadata=None):
        ...
        for i, (sol, obj, beh, meta, sol_origin, e_origin) in enumerate(loop_data):
            fields = {'meta':meta}
            add_info = self.archive.add_single(sol, obj, beh, **fields)
    ...
    def _get_deterministic_elite(self, n=1):
        # Take the best elite
        performances = [e['objective'] for e in self.archive]
        elites = [e for e in self.archive]

        if n == 1:
            i_best = np.argmax(performances)
            elite_float_w = elites[i_best]['solution']
            sample_size = elites[i_best]['meta']
            return [(elite_float_w, sample_size)]
        else:
            return [(elites[i_best]['solution'], elites[i_best]['meta']) for i_best in np.argpartition(performances, -n)[-n:]]
