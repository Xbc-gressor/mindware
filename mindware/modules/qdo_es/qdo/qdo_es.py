...
from mindware.modules.qdo_es.wrapper.abstract_weighted_ensemble import AbstractWeightedEnsemble
from mindware.modules.qdo_es.qdo.behavior_space import BehaviorSpace
from mindware.modules.qdo_es.util.metrics import AbstractMetric

from ribs.schedulers import Scheduler
from mindware.modules.qdo_es.qdo.emitters import DiscreteWeightSpaceEmitter
from mindware.modules.qdo_es.qdo.custom_archives.quality_archive import QualityArchive
# from mindware.modules.qdo_es.qdo.custom_archives.custom_sliding_boundaries_archive import SlidingBoundariesArchive
...
class QDOEnsembleSelection(AbstractWeightedEnsemble):
    ...
    def __init__(self, base_models: List[Callable], configs:List[dict], n_iterations: int, score_metric: AbstractMetric,
        ...
        self.qdo_base_models_metadata_ = configs  # type: Optional[List[dict]]

        all_numeric_keys = []
        for model in base_models:
            cs = model.get_hyperparameter_search_space()
            for hp in cs.get_hyperparameters():
                if hasattr(hp, 'lower') and hasattr(hp, 'upper'):
                    all_numeric_keys.append((hp.name, [hp.lower, hp.upper]))
        ...
        for k, v in all_numeric_keys:
            ...
            if v[0] < curr_min:
                key_to_bounds[k][0] = v[0]
            if v[1] > curr_max:
                key_to_bounds[k][1] = v[1]
    ...
    def _init_archive(self):
        ...
        if self.archive_type == "sliding":
            ...
            archive = SlidingBoundariesArchive(
                solution_dim= self.n_base_models,
    ...
    def _optimize(self, predictions: List[np.ndarray], labels: np.ndarray) -> None:
        # -- Build Optimizer
        opt = Scheduler(self.archive, self.emitters)
    ...
    def _compute_weights(self, predictions, labels):
        ...
        performances = [e['objective'] for e in elites]

        # - Get merged weights
        if self.discrete_merge_:
            disc_weights = np.array([elite['solution'] * elite['meta'] for elite in elites])
            merged_weights = np.sum(disc_weights, axis=0) / np.sum(np.sum(disc_weights, axis=1))
        else:
            merged_weights = np.array([elite['solution'] for elite in elites]).sum(axis=0) / len(elites)
        ...
        if merge_obj > np.max(performances):
            ...
            self.weights_ = elites[np.argmax(performances)]['solution']
