...
class RankNetAdvisor(BaseAdvisor):
    ...
    def fit(self, **kwargs):
        ...
        meta_learner_filename = os.path.join(meta_learner_dir, 'ranknet_model_%s_%s_%s.pth' % (
        ...
        if os.path.exists(meta_learner_filename):
            # print("load model...")
            self.model = torch.load(meta_learner_filename, map_location=self.device)
        else:
            # print("fit model...")
            self.model = RankNet(X1.shape[1], (l1_size, l2_size,), (act_func, act_func,)).to(self.device)
            ...
            for epoch in range(epochs):
                ...
                for i, (data1, data2, y_true) in enumerate(train_loader):
                    ...
                    train_acc += sum(y_pred.detach().cpu().numpy().round() == y_true.detach().cpu().numpy())
