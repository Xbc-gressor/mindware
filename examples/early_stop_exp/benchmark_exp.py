# reshuffle--每个超参数组合都重排一次训练-验证集的组合（split的seed设置为不同的值）
...
sys.path.append('/home/liuwei/mindware')
import pandas as pd
import argparse
...
if '__main__' == __name__:
    ...
    parser.add_argument('--Opt', type=str, default='hpo', help='cash or cashfe')
    ...
    parser.add_argument('--reshuffle', type=str, default=False)
    parser.add_argument('--min_iter', type=int, default=10)
    ...
    for dataset in chosen_datasets:
        ...
        if args.Opt in ['cash', 'cashfe']:
            opt = OPT(
                ...
                reshuffle = args.reshuffle,  resampling_params = {'folds':FOLDS, 'min_iter':MIN_ITER}
            )
        ...
        opt.get_conf(save=True)  # 保存设置

        print(opt.run())
        model_info = opt.get_model_info(save=True)
        opt.get_model_info(save=True)
        scorer = opt.metric
        ...
        if args.evaluation == 'cv':
            with open(args.output_file, 'a+') as f:
                f.write(f'CLS: {args.Opt}-{args.optimizer}, best_estimator:{model_info["best"][0]}, evaluation:{args.evaluation}, folds:{FOLDS}, min_iter:{MIN_ITER}, time:{formatted_start_time}, {args.estimator_id}, reshuffle:{args.reshuffle},  time_limit:{args.time_limit}, {dataset}: {perf}, {ens_perf}\n')
        else:
            with open(args.output_file, 'a+') as f:
                f.write(f'CLS: {args.Opt}-{args.optimizer}, best_estimator:{model_info["best"][0]}, evaluation:{args.evaluation}, time:{formatted_start_time}, {args.estimator_id}, reshuffle:{args.reshuffle}, time_limit:{args.time_limit}, {dataset}: {perf}, {ens_perf}\n')
        print(f'{dataset}: {perf}, {ens_perf}')
