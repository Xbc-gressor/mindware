...
if __name__ == '__main__':
    ...
    parser.add_argument('--kaggle_id', type=str, default="mercedes-benz-greener-manufacturing")
    ...
    parser.add_argument('--metric', type=str, default='r2', help='metric')
    ...
    parser.add_argument('--time_limit', type=int, default=7200, help='time limit')
    parser.add_argument('--per_run_time_limit', type=int, default=300, help='time limit')
    ...
    # with zipfile.ZipFile(f"{kaggle_id}/train.csv.zip", "r") as zip_ref:
    #     zip_ref.extractall(f"{kaggle_id}/train")

    # with zipfile.ZipFile(f"{kaggle_id}/test.csv.zip", "r") as zip_ref:
    #     zip_ref.extractall(f"{kaggle_id}/test")
    dm = DataManager()
    train_csv = dm.load_csv(os.path.join(kaggle_id, 'train.csv'))
    test_csv = dm.load_csv(os.path.join(kaggle_id, 'test.csv'))
    def letter_to_base26(s):
        return sum((ord(char) - ord('a') + 1) * (26 ** i) for i, char in enumerate(reversed(s.lower())))

    # for c in train_csv.columns:
    #     if train_csv[c].dtype == 'object':
    #         train_csv[c] = train_csv[c].apply(letter_to_base26)
    #         test_csv[c] = test_csv[c].apply(letter_to_base26)

    for c in train_csv.columns:
        if train_csv[c].dtype == 'object':
            lbl = LabelEncoder()
            lbl.fit(list(train_csv[c].values) + list(test_csv[c].values))
            train_csv[c] = lbl.transform(list(train_csv[c].values))
            test_csv[c] = lbl.transform(list(test_csv[c].values))
