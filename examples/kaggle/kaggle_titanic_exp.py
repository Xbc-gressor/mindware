...
if __name__ == '__main__':
    ...
    parser.add_argument('--kaggle_id', type=str, default="spaceship-titanic")
    ...
    train_csv[['col1', 'col2', 'col3']] = train_csv['Cabin'].str.split('/', expand=True)
    train_csv = train_csv.drop(columns=['Cabin'])
    train_data_node = dm.from_train_df(train_csv, drop_index=[0, 11], label_col=-4)
    train_data_node = dm.preprocess_fit(train_data_node, task_type)

    test_csv = dm.load_csv(os.path.join(kaggle_id, 'test.csv'))
    test_csv[['col1', 'col2', 'col3']] = test_csv['Cabin'].str.split('/', expand=True)
